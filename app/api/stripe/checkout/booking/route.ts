...
export async function POST(request: Request) {
  ...
  const vatBreakdown = calculateVatBreakdown(body.session.price);
  const payment = calculatePaymentBreakdown(
    vatBreakdown.grossAmount,
    body.session.platformFeePercent,
    body.feeHandling ?? "fees_on_top",
  );
  ...
  const payload = buildPendingBookingPayload({
    session: {
      ...
      price: body.session.price,
      capacity: 0,
      ageRange: "",
      providerStripeAccountId: body.session.providerStripeAccountId ?? "",
      platformFeePercent: body.session.platformFeePercent,
      bookings: 0,
  ...
  const platformFeePence = Math.round(payment.platformFee * 100);
